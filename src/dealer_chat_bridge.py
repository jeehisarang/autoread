...
class DealerChatBridge:
    def __init__(
        ...
        # AI 카드 인식은 핸드/스트리트당 한 번만 "제출"한다(성공/실패 무관, 다시
        # 시도하지 않는다 - task.md "과도한 호출 금지"). hand_no는 계속 증가만
        # 하므로 리셋 없이 멤버십만 확인해도 된다.
        self._hole_cards_attempted_hand: int | None = None
    ...
    def _submit_hero_cards(self, table_img, hand_no: int) -> None:
        """새 핸드당 최대 1회, 히어로 홀카드 AI 호출을 스레드풀에 제출한다(즉시
        반환 - 메인 루프를 안 막음). 결과는 _on_hero_cards_done에서 나중에 찍힌다."""
        if table_img is None or self._hole_cards_attempted_hand == hand_no:
            return
        self._hole_cards_attempted_hand = hand_no
    ...
    def _on_hero_cards_done(self, future, hand_no: int) -> None:
        try:
            ...
            cards = ""
        # 관전 모드라 안 보이면 card_reader가 빈 문자열을 반환하고, 그대로
        # "(미확인)"으로 표시한다(에러 아님 - task.md 3절).
    ...
    def _run(self) -> None:
        ...
        while not self._stop_flag.is_set():
            ...
            for item in self._recorder.process_frame(chat_img, hero_name=self._hero_name):
                ...
                if item.tag == "hand_header":
                    # 핸드 헤더 줄을 먼저 찍은 다음, 그 아래에 홀카드 줄을 잇는다
            ...
            if (
                ...
                if local_count > self._cur_hand_board_count:
                    for count, street in BOARD_COUNT_MILESTONES:
                        if local_count >= count and street not in self._cur_hand_headers:
                            self._cur_hand_headers.add(street)
                            self._emit_line(f"[{street}] 보드: {count}장", "street_header")
                            self._submit_board_cards(table_img, self._cur_hand_no, street)
